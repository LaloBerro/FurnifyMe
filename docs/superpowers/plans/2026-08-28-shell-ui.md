...
.\build\RelWithDebInfo\gui_smoke.exe .
```
Expected: `PASS (0 failures)`, now 29 checks.
...
**These connects go AFTER `buildOverlay();`, not here.** `myItemsPanelAction` is
created by `buildActions()`, which runs later in the constructor — connecting it at
this point would bind a null pointer. Add immediately after the `buildOverlay();`
line:
...
.\build\RelWithDebInfo\gui_smoke.exe .
```
Expected: `PASS (0 failures)`, 43 checks.
...
.\build\RelWithDebInfo\gui_smoke.exe .
```
Expected: `PASS (0 failures)`, 46 checks.
